...
public class MyArrayList<V> implements SaxList<V>, SaxSearchable<V>, SaxSortable<V> {
    ...
    public boolean isSorted(Comparator<V> comparator) {
        ...
        for (int i = 1; i < size; i++) {
            var second = get(i);
            if(second == null && previous!= null){
                return false;
            }
            if(previous != null && comparator.compare(previous, second) > 0){
                return false;
            }
            previous = second;
        }
    ...
    private int splitInPlace(Comparator<V> comparator, int begin, int end) {
        ...
        while (left < right){
            if (index == right) {
                // are we checking on the right side?

                if (comparator.compare(get(index), pivot) > 0) {
                    index = right = right - 1;
                } else {
                    set(left, get(index));
                    index = left = left + 1;
    ...
    public int binarySearch(Comparator<V> comparator, V element) {
        ...
        while(left <= right){
            ...
            if(comparator.compare(get(middle), element) == 0){
                return middle;
            }

            else if(comparator.compare(get(middle), element) < 0){
                left = middle + 1;
            }
            else{
                right = middle - 1;
